...
for (const [slug, content] of entries) {
  const chunks = chunkMarkdown(slug, content);
  allChunks.push(...chunks);
}

const chunksPath = join(vectorsOutputDir, "chunks.ts");
const chunksLines = [
  "// AUTO-GENERATED by apps/mcp/scripts/gen-knowledge.ts — Phase 3: Chunking",
  "// Do not edit manually — run: npm run gen-knowledge",
  "// This file is gitignored; it is rebuilt before every deploy.",
  "",
  ...
  "};",
  "",
  "export const CHUNKS: Record<string, GeneratedChunk> = {",
];

for (const chunk of allChunks) {
  chunksLines.push(`  ${JSON.stringify(chunk.id)}: ${JSON.stringify(chunk, null, 2)},`);
}

chunksLines.push("};" , "");

await writeFile(chunksPath, chunksLines.join("\n"), "utf-8");
console.log(`✓ Generated ${allChunks.length} chunks → src/search/chunks.ts`);
