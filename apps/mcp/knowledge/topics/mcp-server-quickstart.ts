...
const topic: Topic = {
  ...
  manualFacts: [
    ...
    'mcp:auto:flash command: runs generate using Gemini-3 flash with the encrypted ai.json.enc config, then gen-knowledge. It requires CRYPTOKEN environment variable for decryption.',
    'gen-knowledge.ts (apps/mcp/scripts/gen-knowledge.ts) reads generated .md files from the mcp/ directory at repo root and builds the knowledge.ts, bm25-index.ts, chunks.ts and vectors.ts modules used by the MCP server at runtime.',
