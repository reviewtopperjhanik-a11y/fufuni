...
Examples:
npx tsx scripts/generate-static-mcp-response.ts --topic=migrations
npx tsx scripts/generate-static-mcp-response.ts --topic=migrations --topic=do-schemas
npx tsx scripts/generate-static-mcp-response.ts --discover-models
npx tsx scripts/generate-static-mcp-response.ts --dry-run
npx tsx scripts/generate-static-mcp-response.ts --force
AI_MODEL="" AI_API_KEY="" AI_API_URL="" npx tsx scripts/generate-static-mcp-response.ts --auto-refresh --provider=gemini
`);
...
`),
  ...
  {
    name: 'e2e-testing',
    description: 'Playwright e2e test infrastructure, projects, fixtures, selectors, known patterns',
    sources: [
...
`,
    manualFacts: [
      'Test projects: public-chromium, public-firefox, public-mobile (Chromium override — postinstall only installs chromium+firefox, so iPhone 14 uses defaultBrowserType: "chromium"), auth-user (depends on setup-user), auth-admin (depends on setup-admin).',
      'Storage-state fixtures are JSON files exported from the app\'s UserTechnicalInfoModal (PlaywrightSessionExport feature). storage-state-admin.json is used by auth-admin; storage-state-user.json by auth-user. Both live in e2e/fixtures/.',
      'Setup projects (setup-user, setup-admin) validate that the fixture JSON exists and contains an @@auth0spajs@@ key in localStorage — they do NOT perform a browser login.',
      'If storage-state-user.json and storage-state-admin.json contain the same Auth0 token (same account exported twice), the "standard user cannot access admin" test skips itself gracefully with test.skip().',
      'Playwright locale is set to fr-FR and timezoneId to Europe/Paris, but the app detects the user\'s preferred language separately — it currently renders in English. Use bilingual regex selectors (e.g. /ajouter au panier|add to cart/i) for resilience.',
      'Cookie-consent modal (HeroUI Modal, data-slot="modal-backdrop") appears on first visit and blocks all interactions. Always call dismissCookieBanner() at the start of each test. Click "Accept All" and wait for the modal-backdrop to be hidden.',
      'Cart drawer (data-slot="drawer-backdrop") opens automatically after addItem(). Close it with page.keyboard.press("Escape") and wait for drawer-backdrop to be hidden before clicking the cart button again.',
      'The category name column in the HeroUI Table uses role="rowheader", NOT role="gridcell". Use getByRole("rowheader", { name: /.../ }) to assert category names. The handle column is a gridcell.',
      'Action icon buttons (edit/delete) in admin tables are wrapped in a <div title="Edit"> / <div title="Delete"> without an aria-label on the button itself. Use row.locator(\'[title="Delete"] button, [title="Supprimer"] button\').click() to target them.',
      'After creating or deleting a category in admin tests, call page.reload() and waitForLoadState("networkidle") before asserting. The GET /v1/categories endpoint is KV-cached (X-KV-Cache: HIT); React Query invalidateQueries may refetch stale data in local dev.',
      'Modal submit buttons use force: true to bypass backdrop z-index interception during animations. createBtn.click({ force: true }) is safe here.',
      'CSS attribute selectors (input[placeholder="t-shirts"]) are exact and case-sensitive — preferred over getByPlaceholder() which is case-insensitive and does substring matching, leading to ambiguous matches.',
      'Stripe Checkout tests: the worker redirects to checkout.stripe.com (hosted Checkout), NOT Stripe.js Elements. There are no Stripe iframes in the SPA DOM. Tests that require a Stripe redirect skip gracefully when Stripe is not configured (test.skip(true, ...)).',
      'Base URL: PLAYWRIGHT_BASE_URL env var > http://localhost:5173 (PLAYWRIGHT_ENV=local) > https://sctg-development.github.io/fufuni/ (default/CI).',
      'fullyParallel is false; workers: 1 locally, 2 in CI; retries: 0 locally, 2 in CI.',
    ],
    buildPrompt: (src) => appendFacts(`
Below are the Playwright configuration and all current test files.

${src}

Task: Write an "E2E Testing" reference for Fufuni developers.
Include:
1. Project layout: test directories, projects, fixture files, setup/teardown pattern.
2. How to export a storage-state fixture from the app and wire it into a new project.
3. Selector strategy: ARIA-first with bilingual fallbacks, CSS attribute selectors for exact matches, rowheader vs gridcell in HeroUI tables.
4. Cookie consent and cart drawer — the two recurring interaction blockers and how to handle them.
5. KV cache caveat in admin tests and the page.reload() workaround.
6. How to add a new spec file (public / auth-user / auth-admin).
`),
  },
...
async function main() {
  ...
  for (const topic of topics) {
    ...
    // Build the file: dynamic header (with AI metadata and checksums when available) +
